...
<?php if (@validarSessao()) : ?>
  <?php
  $consulta = $conn->query("SELECT * FROM locais");
  $editPermission = havePermission('Locais', 'Listar', 'r');
      ...
      <tbody>
        <?php foreach ($consulta->fetchAll(PDO::FETCH_ASSOC) as $linha) : ?>
          ...
          <tr>
            ...
            <?php if ($editPermission) : ?>
              <td headers='edit'>
                <a href="/sistema_corno/people/personedit.php?id=<?= $linha['id'] ?>" alt='Editar' title='Editar' class='campo-tabela jogar-para-direita'><img src='/sistema_corno/assets/img/edit.png' <?= $editHidden ?> /></a>
              </td>
            <?php endif; ?>
            <?php if ($deletePermission) : ?>
              <td headers='delete'>
                <a href="/sistema_corno/people/persondelete.php?id=<?= $linha['id'] ?>" alt='Exluir' title='Excluir' class='campo-tabela jogar-para-direita'><img src='/sistema_corno/assets/img/delete.png' <?= $deleteHidden ?> /></a>
              </td>
            <?php endif; ?>
          </tr>
        <?php endforeach; ?>
